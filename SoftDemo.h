...
public:
	...
	bool spin_switch; //spin switch
	bool position_switch; //spin switch
	int spin_rate; // spin rate
	int force; // vector force rate
	float softbody_scale;
...
public:
	...
	// 2013/08/28 add inleap control
	void leapControl(SoftDemo* pdemo,Leap::Vector dir,Leap::Vector pos,float spm,float width,bool is_finger);
	// 2013/09/11 add in camera control by leap
	void leapCameraControl(float x);
	
	//2013/10/18 load texture test
	void SoftDemo::load_bmp();
