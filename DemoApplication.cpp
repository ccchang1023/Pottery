...
//--------------for displaying on list , but have no funcitonal relative with softdemo.cpp and main.cpp---------//
bool detection_switch = false;
bool spin_switch = false; //spin switch
int spin_rate=5; // spin rate
int force = 600;
float softbody_scale=0.4f;
//--------------------------------------------------------------------------------------------------------------//
...
void DemoApplication::myinit(void)
{
	...
	GLfloat   mat_flash[]       = {201.0/255.0,119.0/255.0,43.0/255.0,1.0};
    GLfloat   mat_flash_shiny[] = {50.0};
	//GLfloat   light_position[]  = {100.0,200.0,200.0,0.0};
    GLfloat   ambi[]            = {0.1, 0.1, 0.1, 0.1};
    GLfloat   lightZeroColor[]  = {0.0, 0.9, 0.9, 1};
	...
	GLfloat diffuse_light[] = {0.225f,0.225f,0.225f,1.0f};
	...
	GLfloat   light6_position[]  = {0.9,-0.9,-1.0,0.0};
	GLfloat   light7_position[]  = {-0.9,-0.9,-1.0,0.0};
	...
	//glLightfv(GL_LIGHT1, GL_SPECULAR, lightZeroColor);
...
void DemoApplication::testmouseFunc(int button, int state, int x, int y)

{
	...
	//do not call outside core input callback cause by bellow code
	/*updateModifierKeys();
	if ((m_modifierKeys& BT_ACTIVE_ALT) && (state==0))
	{
		return;
...
void DemoApplication::forceInsert(bool dw,bool sw,int sr,int fc,float ss)
{
	detection_switch = dw;
	spin_switch = sw;
	spin_rate = sr;
...
void DemoApplication::showProfileInfo(int& xOffset,int& yStart, int yIncr)
{
	char blockTime[256];
	m_profileIterator->First();
	sprintf(blockTime,"---------Menu------------");
	displayProfileString(xOffset,yStart,blockTime);
	yStart += yIncr;
	sprintf(blockTime,"Q->Quit    R->Restart");
	displayProfileString(xOffset,yStart,blockTime);
	yStart += yIncr;
	sprintf(blockTime,"E->Start/Stop Leap detecting");
	displayProfileString(xOffset,yStart,blockTime);
	yStart += yIncr;
	sprintf(blockTime,"S->Start/Stop Spining [,]->Increase left/right spining force");
	displayProfileString(xOffset,yStart,blockTime);
	yStart += yIncr;
	/*sprintf(blockTime,"U->Change floor  O->Vision Horizonized");
	displayProfileString(xOffset,yStart,blockTime);
	yStart += yIncr;*/
	sprintf(blockTime,"-,= ->enlarge/narrow the scale of pottery");
	displayProfileString(xOffset,yStart,blockTime);
	yStart += yIncr;
	sprintf(blockTime,"Detection switch: %s    Spin Switch: %s    Spin rate: %d units    force: %dunits",(detection_switch)?"on":"off",(spin_switch)?"on":"off",spin_rate,force);
	displayProfileString(xOffset,yStart,blockTime);
	yStart += yIncr;
}
